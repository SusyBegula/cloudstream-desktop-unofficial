...
object PlayerLinkHandler {
    ...
    fun writeMpvConfig(
        ...
        streamKind: StreamKind = StreamKind.PROGRESSIVE,
    ): File {
        val file = PlayerCacheManager.getTempFile("mpv_stream_", ".conf")
        file.deleteOnExit()
        file.writeText(buildMpvConfigContent(headers, subtitles, audioTracks, streamKind))
        return file
    }
    ...
    fun buildMpvConfigContent(
        ...
        return buildString {
            ...
            appendLine("hr-seek=no")
            appendLine("cache=yes")
            appendLine("cache-pause=yes")
            appendLine("cache-pause-wait=1")
            appendLine("demuxer-max-bytes=150M")
            appendLine("demuxer-max-back-bytes=0")
            appendLine("demuxer-readahead-secs=20")
            appendLine("network-timeout=15")
            appendLine("script-opts=osc-seekbarkeyframes=yes")

            if (streamKind == StreamKind.PROGRESSIVE) {
                appendLine("demuxer-lavf-o-append=reconnect=1")
                appendLine("demuxer-lavf-o-append=reconnect_streamed=1")
                appendLine("demuxer-lavf-o-append=reconnect_on_http_error=5xx")
